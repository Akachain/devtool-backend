...
const fs = require('fs');
const loggerComon = require('../utils/logger.js');
const logger = loggerComon.getLogger('channel-service');
...
async function registerUser(req) {
  ...
  const user = username || orgName;
  try {
    const response = await fabricClient.registerUser(user, orgName, true);
    return response;
  } catch (err) {
    logger.error(`Failed to register user ${user} for the ${orgName}: ${err.stack ? err.stack : err}`);
    throw new Error(`Failed to register user ${user} for the ${orgName}: ${err.message}`);
  }
}
...
async function channels(req) {
  ...
  try {
    ...
    const client = await fabricClient.getClientForOrg(orgName);
